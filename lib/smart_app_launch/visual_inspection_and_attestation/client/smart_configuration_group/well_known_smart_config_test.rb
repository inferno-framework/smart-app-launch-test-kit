module SMARTAppLaunch
  class WellKnownSMARTConfigurationAttestationTest < Inferno::Test
    ...
    description %(
      The client application discovers the EHR FHIR server's configuration metadata by appending
    ...
    input :well_known_smart_config_correct,
          title: 'Discovers the server\'s metadata with the .well-known/smart-configuration endpoint',
          description: %(
    ...
    input :well_known_smart_config_note,
          title: 'Notes, if applicable:',
          type: 'textarea',
          optional: true

    run do
      assert well_known_smart_config_correct == 'true',
             'Client application does not discover the EHR FHIR server\'s metadata with
             .well-known/smart-configuration.'
      pass well_known_smart_config_note if well_known_smart_config_note.present?
    end
  end
end
