module SMARTAppLaunch
  class ScopeRequestsAttestationTest < Inferno::Test
    title 'Complies with requirements for scope requests'
    id :scope_requests
    description %(
      The client application complies with requirements for scope requests:
    ...
    input :scope_requests_correct,
          title: 'Complies with requirements for scope requests',
          description: %(
            I attest that the client application complies with requirements for scope requests:
            - Includes a query string suffix to existing scopes, starting with `?` and followed by a series of
              `param=value` items separated by `&` in requests for a scope that applies to a subset of instances of a
              resource type
            - Follows the scope language which is the following sequence of characters
              - one of "patient", "user", or "system"
              - either a FHIR resource type or "*"
              ...
              - optional "d"
              - optional "s"
              - optional "?" followed by at least 1 "<param>=<value>" pairs, where <param> is a valid search parameter
                and <value> is a valid corresponding value, with each pair each separated by "&" if there are multiple
            - Starts the query string with `patient/` for requests for patient-specific scopes
            - Starts the query string with `user/` for requests for user-level scopes
            - Starts the query string with `system/` for requests for system-level scopes
            - Includes a wildcard (`*`) for the FHIR resource for Wildcard scopes
    ...
    input :scope_requests_note,
          title: 'Notes, if applicable:',
          type: 'textarea',
          optional: true

    run do
      assert scope_requests_correct == 'true',
             'Client application does not comply with requirements for scope requests.'
      pass scope_requests_note if scope_requests_note.present?
    end
  end
end
