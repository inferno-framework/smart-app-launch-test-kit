  ...
  describe 'capability statement test' do
    ...
    it 'passes when all required extensions are present' do
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 200, body: full_capabilities.to_json, headers: cors_header)
    ...
    it 'passes when all required extensions are present with relative URLs' do
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 200, body: relative_capabilities.to_json, headers: cors_header)
    ...
    it 'fails when a non-200 response is received' do
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 500, body: minimal_capabilities.to_json, headers: cors_header)
    ...
    it 'fails when a response with no cors headers is received' do
      stub_request(:get, "#{url}/metadata")
    ...
    it 'fails when a response with incorrect cors headers is received' do
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 200, body: minimal_capabilities.to_json, headers: incorrect_cors_header)
    ...
    it 'fails when no SMART extensions are returned' do
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 200, body: minimal_capabilities.to_json, headers: cors_header)
    ...
    it 'fails when no authorize extension is returned' do
      ...
      stub_request(:get, "#{url}/metadata")
        .to_return(status: 200, body: capabilities_with_smart(extensions).to_json, headers: cors_header)
  ...
  describe 'endpoints match test' do
    ...
    let(:full_inputs) do
      [
        ...
        inputs[:"well_known_#{type}_url"] = "#{type.upcase}_URL"
        inputs[:"capability_#{type}_url"] = "#{type.upcase}_URL"
      end
    end
